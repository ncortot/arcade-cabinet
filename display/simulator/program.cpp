...
void Program::loop()
{
    ...
    const rgb24 lineColor[] = {{0xFC, 0xCC, 0x2E}, {0xEC, 0x39, 0x94}, {0xAE, 0x4F, 0x93}, {0x31, 0x88, 0xBE}, {0x21, 0xAB, 0x77}};
    const int length = 60;
    ...
    while (true) {
        ...
        for (int i = 0; i < strlen(text); i++) {
            ...
            rgb24 color;
            color.red = startGradient.red + (endGradient.red - startGradient.red) * (xCur + apple6x10.Width) / (MATRIX_WIDTH + apple6x10.Width);
            color.green = startGradient.green + (endGradient.green - startGradient.green) * xCur / MATRIX_WIDTH;
            color.blue = startGradient.blue + (endGradient.blue - startGradient.blue) * xCur / MATRIX_WIDTH;
